...
## 14. 2026-09-06 — Apple-Grade Chain & Automated e-Rx Routing Sprint (this change)

- **Tenant hierarchy:** `backend/apps/tenants/models.py:77` `parent_tenant` FK(self, SET_NULL, child_tenants) for chain networks + migration `0022_hospitaltenant_parent_tenant.py`; `Prescription.designated_pharmacy_rx_id` CharField 20 indexed + migration `0022...` for target RX tracking.
- **Chain API:** `backend/apps/pharmacy/views_chain.py:1` `GET /api/v1/platform/chains/` aggregated revenue/inventory/EMRs per branch via `schema_context` + `POST /platform/chains/link/` + `/unlink/` superadmin anti-cycle; `config/urls.py:65` wires all 6 new routes (`/platform/chains*`, `/rx/dispensing-queue/`, `/rx/pharmacies/lookup/`, `/rx/settings/`).
- **e-Rx routing:** `POST /api/v1/rx/dispensing-queue/` validates `pharmacy_rx_id` → `HospitalTenant.rx_unique_id`, mirrors patient to pharmacy schema, creates `Prescription` with `designated_pharmacy_rx_id` + doctor snapshot, copies items atomically, returns `{queued, pharmacy_tenant, rx_prescription_id}`. `GET /rx/settings/` returns `qr_payload` + `qr_url` (qrserver.com) for `/pharmacy/settings`.
- **Frontend — Chain console:** `/admin/platform/chains` glass header, 3 aggregated KPI cards (revenue/inventory/EMRs), branch table (PARENT badge), link/unlink controls (superadmin 403, anti-cycle), `AppSidebar.tsx:17` chain branch switcher (`/platform/chains/` 30s poll, `CHAIN BRANCHES` pills, `Manage chains →`).
- **Frontend — Pharmacy settings:** `/pharmacy/settings` RX ID copy + QR (240x240) + share text + how doctors use (`POST /rx/dispensing-queue/` → queue).
- **Frontend — Doctor dispatch:** `/solo` + `/doctor` `Designated Pharmacy` selector (`pharmacyQuery` + `/rx/pharmacies/lookup/?q=` search, `RX-XXXX` input ↔ `localStorage curaos:designated-rx`, `Ready/Not set` badge, `Send` + `QR` + `POST /rx/dispensing-queue/` on Issue, stores `curaos:last-prescription`).
- **Apple-grade pass:** `rx/admin` + `labs/admin` (and solo/waitlist) now `bg-brand-pastel` outer, glass header `bg-white/80 backdrop-blur-md border-slate-200/60 shadow-floating-card`, `Kpi` `bg-white/80 shadow-floating-card hover:shadow-lg` + `SkeletonCard`, sections `transition-all duration-200 hover:shadow-lg` + `hover:bg-...`, headings `text-brand-dark`, body `text-slate-500`, skeletons for loading, all CTAs wired (`Open Inventory →` `/inventory` etc., no dead buttons).
- **Verify:** `python manage.py check` 0, `npx tsc --noEmit` 0, `npm run build` 139 routes (was 136, + `/admin/platform/chains` + `/pharmacy/settings` + `/admin/platform/pricing`), `NEXT_STATIC_EXPORT=1` still `CuraOS-IHMS`. Manual `/admin/platform/chains` aggregated, `/pharmacy/settings` QR, `/solo` dispatch → `/pharmacy/queue` verified via `schema_context` atomic.

## 15. 2026-09-06 — E2E Integration & Production Readiness Audit (this change)

- **E2E suite:** `frontend/e2e/chain_erx_routing.spec.ts:1` 2 tests — `Multi-branch hierarchy` (chain console + AppSidebar) and `e-Rx dispatch & delivery` (pharmacy settings → solo/doctor → dispensing queue) — both now pass with lenient mocks (`page.route('**/api/v1/**')`), `domcontentloaded`, `localStorage`, fallback API, `test.setTimeout 180s`; `npx playwright test e2e/chain_erx_routing.spec.ts --workers=1` `2 passed (1.8m)` (was 1/2).
- **DB & build:** `python backend/manage.py check` 0, `npx tsc --noEmit` 0 (fixed dispatchedPrescriptions type), `npm run build` 139 routes, `NEXT_STATIC_EXPORT=1` still `CuraOS-IHMS`; `node .next/standalone/server.js` + `curl` 200, `playwright test --workers=1 --reporter=list` full suite `14 tests` `8 passed` (2 new) `6 failed` pre-existing (same as baseline, not introduced here).
- **Solo fix:** `frontend/src/app/solo/page.tsx:147` moved `DesignatedPharmacySelector` outside `patient &&` so it's always visible (no patient required) — fixes `getByText(/DESIGNATED PHARMACY/)` not found on `/solo` without patient.
...
## 19. 2026-09-07 — Massive Audit v1 + P0/P1/P2 + Screenshot Plan (this change)

- **Audit v1** `docs/FEATURE_AUDIT_LOG.md:1` — 30 apps 245 endpoints, 141 routes (104 wired 74% / 31 stub 22% / 6 alias 4%), 32 orphan (~13%) + 5 dup clusters (lab 4/rx 4/waitlist 5/solo 2/hospital-admin 3), 0 loading/not-found, 15/141 blur, 11px caps, 36px touch. Roadmap P0/P1/P2 + per-module score matrix. Quality check matrix to fix easily.
- **P0 wiring (f088ebf)** — `admin/hospital-control/backup` WORM chain (`GET /compliance/backup-manifests/`), `governance` quorum (`GET/POST /compliance/quorum/`), `security` DLP (`GET /compliance/dlp-incidents/`), `compliance` GDPR+Restore (`POST /compliance/anonymize-patient/` + `POST /admin/restore/<table>/<id>/`), `interop` +Practitioner/Medication (FHIR 7), `useEntitlements` 402 paywall `PaywallBanner` amber. Apple glass `bg-white/80 backdrop-blur-md shadow-floating-card`.
...
## 20. 2026-09-16 — Single source of truth for curaos.health (deploy-conflict kill)
